...
#SBATCH --ntasks=1                  # Number of "tasks". For use with distributed parallelism. 
#SBATCH --cpus-per-task=16          # # of CPUs allocated to each task. For use with shared memory parallelism.
#SBATCH --ntasks-per-node=1         # Number of "tasks" per node. For use with distributed parallelism. 
